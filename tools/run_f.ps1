...
$Log = Join-Path $LogDir ("runF_" + $stamp + ".log")
...
try {
  ...
  $script = Join-Path $Root "tools\f_dual_scoring.py"
  ...
  if ($All) {
    ...
    foreach ($d in $dirs) {
      ...
      if ($rc -eq 2) {
        $prompt = Join-Path $IdeaDir2 "out\llm_prompt_F.txt"
        $target = Join-Path $IdeaDir2 "in\llm_stageF.json"
        if (Test-Path $prompt) {
    ...
    if ($failed -gt 0) {
      ...
      LogLine "[OK] Done. Stage F generated for all ideas."
      exit 0
    }
  }
  ...
  if ($rc -eq 2) {
    $prompt = Join-Path $IdeaDir "out\llm_prompt_F.txt"
    $target = Join-Path $IdeaDir "in\llm_stageF.json"
    if (Test-Path $prompt) {
