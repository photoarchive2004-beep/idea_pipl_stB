...
def validate_llm(obj: Dict[str, Any], candidates: List[Dict[str, Any]], run_id: str) -> Tuple[bool, str, List[Dict[str, Any]]]:
    ...
    for x in rows:
        if not isinstance(x, dict):
            continue
        try:
            cid = int(str(x.get("claim_id","")).strip())
        except Exception:
            continue
        sid = str(x.get("source_id","")).strip()
        if not sid:
            continue
        if (cid, sid) not in allowed_pairs:
            continue
        if (cid, sid) in seen:
            continue
        seen.add((cid, sid))

        rel = str(x.get("relation","unclear")).lower().strip()
        if rel not in allowed_rel:
            rel = "unclear"

        cert = str(x.get("certainty","Low")).strip()
